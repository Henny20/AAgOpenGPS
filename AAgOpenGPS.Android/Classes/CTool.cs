//﻿using AgOpenGPS.Core.Drawing; TODO
//using AgOpenGPS.Core.DrawLib; not handy
using AgOpenGPS.Core.Models;
//using OpenTK.Graphics.OpenGL; not available
using System;
using System.IO;
using System.Linq;
using Avalonia.Media;
//using System.Drawing; not useable on Linux/Android
...
namespace AAgOpenGPS.Android
{
    public class CTool
    {
        //private readonly FormGPS mf; hennie
       ...
       // public CTool(FormGPS _f) hennie
         public CTool(Main _f)
        {
            mf = _f; 

            //from settings grab the vehicle specifics
            string connectionString = Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.ApplicationData), "file.db");
            SQLiteConnection connection = new SQLiteConnection(connectionString);
            
            trailingToolToPivotLength = connection.Query<Setting>("SELECT * FROM Setting WHERE Id=1").FirstOrDefault().setTool_trailingToolToPivotLength;//Properties.Settings.Default.setTool_trailingToolToPivotLength;
            width =  connection.Query<Setting>("SELECT * FROM Setting WHERE Id=1").FirstOrDefault().setVehicle_toolWidth; //Properties.Settings.Default.setVehicle_toolWidth;
            Console.WriteLine("toolwidth: " + width);
            overlap = connection.Query<Setting>("SELECT * FROM Setting WHERE Id=1").FirstOrDefault().setVehicle_toolOverlap; //Properties.Settings.Default.setVehicle_toolOverlap;
/***
        ...
        {
            XyCoord[] vertices = {
                new XyCoord(-0.65 + MainViewModel.Instance.tool.offset, trailingTool),
                new XyCoord(0.0, 0.0),
                new XyCoord(0.65 + MainViewModel.Instance.tool.offset, trailingTool)
            };
            LineStyle backgroundLineStyle = new LineStyle(6.0f, Colors.Black);
...
/***********
        public void DrawTool()
        {
            ...
            GL.Translate(MainViewModel.Instance.pivotAxlePos.easting, MainViewModel.Instance.pivotAxlePos.northing, 0);
            GL.PushMatrix();

            //translate down to the hitch pin
            GL.Translate(Math.Sin(MainViewModel.Instance.fixHeading) * (hitchLength),
                            Math.Cos(MainViewModel.Instance.fixHeading) * (hitchLength), 0);
            ...
            if (isToolTBT && isToolTrailing)
            {
                ...
                GL.Rotate(glm.toDegrees(-MainViewModel.Instance.tankPos.heading), 0.0, 0.0, 1.0);
                ...
                MainViewModel.Instance.VehicleTextures.ToolAxle.DrawCentered(toolAxleCenter, deltaToU1V1);

                //move down the tank hitch, unwind, rotate to section heading
                GL.Translate(0.0, trailingTank, 0.0);
                GL.Rotate(glm.toDegrees(MainViewModel.Instance.tankPos.heading), 0.0, 0.0, 1.0);
            }
            GL.Rotate(glm.toDegrees(-MainViewModel.Instance.toolPivotPos.heading), 0.0, 0.0, 1.0);
            ...
            if (isToolTrailing)
            {
                ...
                if (Math.Abs(trailingToolToPivotLength) > 1 && MainViewModel.Instance.camera.camSetDistance > -100)
                {
                    textRotate += (MainViewModel.Instance.sim.stepDistance);
                    GL.Color4(1, 1, 1, 0.75);
                    ...
                    MainViewModel.Instance.VehicleTextures.Tire.Draw(rightTire00, rightTire11);
                    MainViewModel.Instance.VehicleTextures.Tire.Draw(leftTire00, lefttTire11);
                }
                trailingTool -= trailingToolToPivotLength;
            }

            if (MainViewModel.Instance.isJobStarted)
            {
                //look ahead lines
                GL.LineWidth(3);
                ...
                GL.Vertex3(MainViewModel.Instance.tool.farLeftPosition, (MainViewModel.Instance.tool.lookAheadDistanceOnPixelsLeft) * 0.1 + trailingTool, 0);
                GL.Vertex3(MainViewModel.Instance.tool.farRightPosition, (MainViewModel.Instance.tool.lookAheadDistanceOnPixelsRight) * 0.1 + trailingTool, 0);

                //lookahead section off
                GL.Color3(0.70f, 0.2f, 0.2f);
                GL.Vertex3(MainViewModel.Instance.tool.farLeftPosition, (MainViewModel.Instance.tool.lookAheadDistanceOffPixelsLeft) * 0.1 + trailingTool, 0);
                GL.Vertex3(MainViewModel.Instance.tool.farRightPosition, (MainViewModel.Instance.tool.lookAheadDistanceOffPixelsRight) * 0.1 + trailingTool, 0);

                if (MainViewModel.Instance.vehicle.isHydLiftOn)
                {
                    GL.Color3(0.70f, 0.2f, 0.72f);
                    GL.Vertex3(MainViewModel.Instance.section[0].positionLeft, (MainViewModel.Instance.vehicle.hydLiftLookAheadDistanceLeft * 0.1) + trailingTool, 0);
                    GL.Vertex3(MainViewModel.Instance.section[MainViewModel.Instance.tool.numOfSections - 1].positionRight, (MainViewModel.Instance.vehicle.hydLiftLookAheadDistanceRight * 0.1) + trailingTool, 0);
                }
            ...
            double hite = MainViewModel.Instance.camera.camSetDistance / -250;
            if (hite > 4) hite = 4;
            if (hite < 1) hite = 1;
            ...
            for (int j = 0; j < numOfSections; j++)
            {
                ...
                if (MainViewModel.Instance.section[j].isSectionOn)
                {
                    if (MainViewModel.Instance.section[j].sectionBtnState == btnStates.Auto)
                    {
                        //GL.Color3(0.0f, 0.9f, 0.0f);
                        if (MainViewModel.Instance.section[j].isMappingOn) GL.Color3(0.0f, 0.95f, 0.0f);
                ...
                {
                    if (!MainViewModel.Instance.section[j].isMappingOn) GL.Color3(0.950f, 0.2f, 0.2f);
                    else GL.Color3(0.00f, 0.250f, 0.97f);
                    //GL.Color3(0.7f, 0.2f, 0.2f);
                }

                double mid = (MainViewModel.Instance.section[j].positionRight - MainViewModel.Instance.section[j].positionLeft) / 2 + MainViewModel.Instance.section[j].positionLeft;
                XyCoord[] vertices = {
                    new XyCoord(MainViewModel.Instance.section[j].positionLeft, trailingTool),
                    new XyCoord(MainViewModel.Instance.section[j].positionLeft, trailingTool - hite),
                    new XyCoord(mid, trailingTool - hite * 1.5),
                    new XyCoord(MainViewModel.Instance.section[j].positionRight, trailingTool - hite),
                    new XyCoord(MainViewModel.Instance.section[j].positionRight, trailingTool),
                };
                GLW.DrawTriangleFanPrimitive(vertices);

                if (MainViewModel.Instance.camera.camSetDistance > -width * 200)
                {
            ...
            //zones
            if (!isSectionsNotZones && zones > 0 && MainViewModel.Instance.camera.camSetDistance > -150)
            {
                //GL.PointSize(8);
                ...
                for (int i = 1; i < zones; i++)
                {
                    ...
                    GL.Vertex3(MainViewModel.Instance.section[zoneRanges[i]].positionLeft, trailingTool - 0.4, 0);
                    GL.Vertex3(MainViewModel.Instance.section[zoneRanges[i]].positionLeft, trailingTool + 0.2, 0);
                }
            ...
            //tram Dots
            if (isDisplayTramControl && MainViewModel.Instance.tram.displayMode != 0)
            {
                if (MainViewModel.Instance.camera.camSetDistance > -300)
                {
                    if (MainViewModel.Instance.camera.camSetDistance > -100)
                        GL.PointSize(12);
                    else GL.PointSize(8);

                    ColorRgb rightMarkerColor = ((MainViewModel.Instance.tram.controlByte) & 1) != 0 ? Colors.TramMarkerOnColor : Colors.Black;
                    ColorRgb leftMarkerColor = ((MainViewModel.Instance.tram.controlByte) & 2) != 0 ? Colors.TramMarkerOnColor : Colors.Black;
                    double rightX = MainViewModel.Instance.tram.isOuter ? farRightPosition - MainViewModel.Instance.tram.halfWheelTrack : MainViewModel.Instance.tram.halfWheelTrack;
                    double leftX = MainViewModel.Instance.tram.isOuter ? farLeftPosition + MainViewModel.Instance.tram.halfWheelTrack : -MainViewModel.Instance.tram.halfWheelTrack;
                    // section markers
