...
/*** not available
        public void DrawFenceLines()
        {
            ...
            {
                GL.LineWidth(mfABLine.lineWidth * 3);
            ...
            //GL.Vertex3(mfbnd.closestTurnPt.easting, mfbnd.closestTurnPt.northing, 0);
            //GL.End();

            if (bndBeingMadePts.Count > 0)
            {
                //the boundary so far
                vec3 pivot = mfpivotAxlePos;
                GL.LineWidth(mfABLine.lineWidth);
                ...
                else //draw from tool
                {
                    ...
                    {
                        GL.Vertex3(bndBeingMadePts[0].easting, bndBeingMadePts[0].northing, 0);
                        GL.Vertex3(mfsection[mftool.numOfSections - 1].rightPoint.easting, mfsection[mftool.numOfSections - 1].rightPoint.northing, 0);
                        GL.Vertex3(bndBeingMadePts[bndBeingMadePts.Count - 1].easting, bndBeingMadePts[bndBeingMadePts.Count - 1].northing, 0);
                    }
                    else
                    {
                        GL.Vertex3(bndBeingMadePts[0].easting, bndBeingMadePts[0].northing, 0);
                        GL.Vertex3(mfsection[0].leftPoint.easting, mfsection[0].leftPoint.northing, 0);
                        GL.Vertex3(bndBeingMadePts[bndBeingMadePts.Count - 1].easting, bndBeingMadePts[bndBeingMadePts.Count - 1].northing, 0);
                    }
                }
                GL.End();
